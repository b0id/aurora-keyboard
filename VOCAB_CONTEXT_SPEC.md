...
6. **The test suite may never open `/dev/uinput`.** Rule 4 covered *manual* input simulation, but `tests/test_keyboard_window.py` was constructing a real `KeyEngine` and driving `handle_key_click()` through it — so running the suite typed real characters and pressed real Esc/Super into whatever window the user had focused. `key_engine.py` now honours an `AURORA_NO_UINPUT` interlock: with it set, no virtual device is created at all and every emit path no-ops on `self.ui is None`. Every test module that touches `keyboard_window.py` or `KeyEngine` sets it at import time, before any construction. Discovered and fixed 2026-09-13.
7. **Visual/on-screen confirmation is handed back to the user once they're at the machine** — screenshots and tool-based checks are used for what can't otherwise be verified (log files, decode output, process state), not for "does this look right," which the user can just look at.
